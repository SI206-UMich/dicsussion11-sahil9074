...
def create_patients_table(cur, conn):
    ...
    cur.execute('create table Patients(pet_id INTEGER PRIMARY KEY, name TEXT, species_id, NUMBER, age INTEGER, cutness INTEGER, aggressiveness INTEGER)')
    conn.commit()

# ADD FLUFFLE TO THE TABLE
def add_fluffle(cur, conn):
    cur.execute('insert into Patients (pet_id, name, species_id, age, cutness, aggressiveness) values(?, ?, ?, ?, ?, ?)', (0, "Fluffe", 0, 3, 90, 100))
    conn.commit()
    pass
...
def add_pets_from_json(filename, cur, conn):
    ...
    for row in json_data:
        name = row["name"]
        species = cur.execute("Select id from Species WHERE title = ?", row['species'])
